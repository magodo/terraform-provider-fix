...
var AnalysisServicesServer = tfxsdk.ReferenceConfigUpgraders{
		...
		ReferenceConfigUpgrader: func(ctx context.Context, req tfxsdk.UpgradeReferenceConfigRequest, resp *tfxsdk.UpgradeReferenceConfigResponse) {
			...
			for _, tv := range tvs {
				ntv, err := tfxsdk.TraversalReplace(
					tv,
					append(tv[:2], hcl.TraverseAttr{Name: "enable_power_bi_service"}),
					hcl.Traversal{hcl.TraverseAttr{Name: "power_bi_service_enabled"}},
				)
				if err != nil {
					resp.Error = err
					return
